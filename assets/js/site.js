...
// V3 Build 03 — randomized hero diagrams
(() => {
  "use strict";

  const diagrams = [...document.querySelectorAll("[data-hero-diagram]")];
  const selectorButtons = [...document.querySelectorAll("[data-diagram-select]")];
  const randomButton = document.querySelector("[data-diagram-random]");
  const keys = diagrams.map((item) => item.dataset.heroDiagram);

  if (!diagrams.length) return;

  const show = (key, persist = false) => {
    diagrams.forEach((diagram) => {
      diagram.classList.toggle("is-active", diagram.dataset.heroDiagram === key);
    });

    selectorButtons.forEach((button) => {
      button.classList.toggle("active", button.dataset.diagramSelect === key);
    });

    if (persist) {
      sessionStorage.setItem("heroDiagramPreference", key);
    }
  };

  const randomKey = () => keys[Math.floor(Math.random() * keys.length)];

  // Randomize on every new page load unless the visitor chose a diagram in this tab.
  const preferred = sessionStorage.getItem("heroDiagramPreference");
  show(preferred && keys.includes(preferred) ? preferred : randomKey());

  selectorButtons.forEach((button) => {
    button.addEventListener("click", () => {
      show(button.dataset.diagramSelect, true);
    });
  });

  randomButton?.addEventListener("click", () => {
    sessionStorage.removeItem("heroDiagramPreference");
    show(randomKey());
  });
})();
